   ...
   for(i=0; i<8; i++)
    ...
    for(i=0; i<8; i++)
      if((i+q[i])%2 == 0)
         ...
         board[q[i]][i] = &bq;// Set up the current solution on the chessboard by placing pointers to bq and wq in the appropriate squares.
   // Place black queens on white squares and white queens on black squares,to see them when printing.
